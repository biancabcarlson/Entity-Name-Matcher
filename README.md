...
Match names, emails, phones, and addresses across accounts, including previously used contact information. Built for KYC / case-review work: catching duplicate accounts, synthetic identities, or an account takeover where the attacker opens a second account using the victim's *previously used* contact info (an old email or phone number the victim no longer uses).
...
Includes a browser-based live demo (`index.html`, linked above) that runs entirely client-side — nothing is saved or uploaded. It comes pre-loaded with two example accounts and shows any flagged overlap immediately, no manual search needed; there's also a field to search a different name/email/phone/address against all loaded accounts.

Each flagged match shows a masked account number for the associated account it found (e.g. `•••1187`), with a click-to-unmask toggle — the same reveal pattern used elsewhere in this tool series. Match descriptions state *which* field overlapped (email, phone, address) without restating the literal value, since that's already visible once in the accounts table above — avoids showing the same contact info twice.
